...
'''
Part of code credit for https://github.com/zh217/torch-dct.git
'''
...
def _dct_nd(x, dim, dim_cur, norm=None):
    X = dct(x, norm=norm)
    if dim_cur == dim:
        return X
    else:
        dim_cur += 1
        return _dct_nd(X.transpose(-1, -dim_cur), dim, dim_cur, norm=norm).transpose(-1, -dim_cur)


def idct_nd(X, dim, norm=None):
    """
    The inverse to 3D DCT-II, which is a scaled Discrete Cosine Transform, Type III
    Our definition of idct is that idct_3d(dct_3d(x)) == x
    For the meaning of the parameter `norm`, see:
...
def _idct_nd(x, dim, dim_cur, norm=None):
    X = idct(x, norm=norm)
    if dim_cur == dim:
        return X
    else:
        dim_cur += 1
        return _idct_nd(X.transpose(-1, -dim_cur), dim, dim_cur, norm=norm).transpose(-1, -dim_cur)
...
if __name__ == '__main__':
    ...
    assert error.max().item() < 1e-3, (error, error.max())
    linear_idct = LinearDCT(4096, 'idct')
    error = torch.abs(idct(x) - linear_idct(x))
    assert error.max().item() < 1e-3, (error, error.max())

    error = torch.abs(x - linear_idct(linear_dct(x)))
    ...
    error = torch.abs(dct_3d(x) - dct_nd(x, 3))
    assert error.max().item() < 1e-10, (error, error.max())
